...
def get_unlike_count(obj):
    content_type = ContentType.objects.get_for_model(obj)
    unlike_count, created = UnLikeCount.objects.get_or_create(content_type=content_type, object_id=obj.pk)
...
def get_unlike_status(context, obj):
    ...
    if not user.is_authenticated:
        return ''
    if UnLikeRecord.objects.filter(content_type=content_type, object_id=obj.pk, user=user).exists():
        return 'active2'
    else:
        return ''